...
- $\bold{1}$：$\bold{1}$ 函数，若 $f(n)=\bold{1}$，则 $\forall x \in N^+,f(x)=1$
...
即证明 $\phi*\bold{1}=\operatorname{id}$ 。

我们只需要证明 $\phi*\bold{1}$ 在 $p^c$ 处取值为 $\operatorname{id}(p^c)$，由于 $\phi,\bold{1},\operatorname{id}$ 均为积性函数，自然在所有位置成立。
$$
\begin{align}
