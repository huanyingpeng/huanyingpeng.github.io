...
选第一个点的期望次数为 $1$，因为所有满足这个排列的样本点都是以 $1$ 开头，选第二个点，期望次数是多少，或者说，选出第一个点后，下一个被选择的点，在顺序为 $3,2,1$ 的前提下，是 $2$ 的概率是多少？
...
$$
\begin{align}
P(下一个值为目标|满足排列限制)=&\dfrac{P(下一个值合法\wedge满足排列限制)}{P(满足排列限制)}\\
...
**样本空间无线大的时候要小心，不能轻易认为两个元素等价，因为此时 $\infty \ne \infty$**。
