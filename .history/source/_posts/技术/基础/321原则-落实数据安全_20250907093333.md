...
```bash
# 定义配置变量
$SCRIPT_PATH = "$TOOLS_DIR\backup.ps1"
$LOCAL_REPO = $BACKUP_DIR_LOCAL
$REMOTE_REPO = $BACKUP_DIR_REMOTE
$REPO_PASSWORD = $PASSWORD
$REPETITION_HOURS = 1
$TASK_DURATION_DAYS = 3650
$NAS_DELAY_MINUTES = 15

# 1. 本地备份任务 - 每小时
$trigger = New-ScheduledTaskTrigger -Once -At (Get-Date) `
                  -RepetitionInterval (New-TimeSpan -Hours $REPETITION_HOURS) `
...
$action  = New-ScheduledTaskAction -Execute "PowerShell.exe" `
                  -Argument "-WindowStyle Hidden -ExecutionPolicy Bypass -File $SCRIPT_PATH `
                            -Repo $LOCAL_REPO -Pass $REPO_PASSWORD"
...
# 2. NAS备份任务 - 每小时（错开指定分钟数减轻网络突刺）
$trigger2 = New-ScheduledTaskTrigger -Once -At (Get-Date).AddMinutes($NAS_DELAY_MINUTES) `
                   -RepetitionInterval (New-TimeSpan -Hours $REPETITION_HOURS) `
                   -RepetitionDuration (New-TimeSpan -Days $TASK_DURATION_DAYS)
$action2  = New-ScheduledTaskAction -Execute "PowerShell.exe" `
                   -Argument "-WindowStyle Hidden -ExecutionPolicy Bypass -File $SCRIPT_PATH `
                             -Repo $REMOTE_REPO -Pass $REPO_PASSWORD"
